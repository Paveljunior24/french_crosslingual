...
public class AsapStatistics {
	...
	// id -> n -> fd
	private Map<Integer, Map<Integer, FrequencyDistribution<String>>> ngramDistribution;
	...
	public FrequencyDistribution<String> getNgramDistribution(int n, Integer[] ids) {
		if(ids.length == 0) {
			ids = this.getIds().toArray(new Integer[this.getIds().size()]);
		}
		FrequencyDistribution<String> resultNGrams = new FrequencyDistribution<String>();
		for(int id : ids) {
			if(this.vocabulary.get(id).containsKey(n)) {
				for(String key : this.vocabulary.get(id).get(n).getKeys()) {
					resultNGrams.addSample(key, this.vocabulary.get(id).get(n).getCount(key));
				}	
			}
		}
		return resultNGrams;
	...
	public void incrementTotalCharacterCount(int id, int label, String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			// TODO: vernünftigen Segmenter reintun
