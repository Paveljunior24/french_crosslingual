...
public class AsapAnalysis implements CorpusAnalysis {
	...
	public Set<String> getTopKNgrams(int k, int n, Integer...ids){
		return new HashSet<String>(this.asapStatistics.getNgramDistribution(n, ids).getMostFrequentSamples(k));
	}
	...
	public void printVocabDistribution (Integer...ids) throws IOException{
		String idString = "";
		for (int id : ids){
			idString = idString+"_"+id;
		}
		idString = idString.substring(1);
		BufferedWriter bw = new BufferedWriter(new FileWriter("src/main/resources/vocabularyLists/Vocabulary_"+this.dataSetName+"_"+idString+".txt"));
		FrequencyDistribution<String> fd = this.asapStatistics.getVocabDistribution(ids);
		List<String> samplesOrderedByFrequency = fd.getMostFrequentSamples(fd.getKeys().size());
		for (String sample : samplesOrderedByFrequency){
			bw.write(sample + "\t" + fd.getCount(sample));
		}
		bw.close();
	}
			...
			while((line = br.readLine()) != null) {
				if(!line.isEmpty() && Character.isDigit(line.charAt(0))) {
					...
					if(lineArray.length == 5) {
						...
						//remove punctuation and multiple whitespaces
						String[] words = lineArray[3].toLowerCase().replaceAll("\\p{P}", "").replaceAll("\\s+", " ").trim().split(" ");
